...
class FrontendService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self._get_url(endpoint_key), timeout=self.config["timeout"]
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Backend communication failed: {str(e)}"
            )
    ...
    async def post_data(self, endpoint_key: str, payload: dict, files: dict = None):
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Backend communication failed: {e}"
            )
    ...
    async def stream_data(self, endpoint_key: str, payload: dict):
        ...
        try:
            print(self._get_url(endpoint_key))
            async with httpx.AsyncClient() as client:
                request_params = {
                    "url": self._get_url(endpoint_key),
                    "json": payload,
            ...
            raise HTTPException(
                status_code=500, detail=f"Backend streaming failed: {str(e)}"
            )
